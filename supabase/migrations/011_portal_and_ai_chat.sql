...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'client_products' AND policyname = 'service_role_all_client_products') THEN
    EXECUTE 'CREATE POLICY "service_role_all_client_products" ON client_products FOR ALL USING (auth.role() = ''service_role'')';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'client_products' AND policyname = 'anon_read_own_client_products') THEN
    EXECUTE 'CREATE POLICY "anon_read_own_client_products" ON client_products FOR SELECT USING (true)';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'kb_documents' AND policyname = 'service_role_all_kb_documents') THEN
    EXECUTE 'CREATE POLICY "service_role_all_kb_documents" ON kb_documents FOR ALL USING (auth.role() = ''service_role'')';
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'service_role_kb_upload') THEN
    EXECUTE 'CREATE POLICY "service_role_kb_upload" ON storage.objects FOR ALL USING (bucket_id = ''kb-documents'' AND auth.role() = ''service_role'')';
  END IF;
END $$;
