...
function buildKnowledgeContext(query: string): string {
  ...
  }).sort((a, b) => b.score - a.score).filter((s) => s.score > 0).slice(0, 3).map((s) => s.w)
...
async function loadKbDocsForCategory(): Promise<string> {
  try {
    ...
    const { data, error } = await admin.from('kb_documents').select('title, category, content').limit(5)
    if (error || !data) return ''
    return data.filter((d) => d.content && !d.content.startsWith('(')).map((d) => `### ${d.title} (${d.category})\n${(d.content as string).slice(0, 3000)}`).join('\n\n---\n\n')
  } catch { return '' }
}

export async function POST(req: NextRequest) {
  if (!process.env.GEMINI_API_KEY) {
    return NextResponse.json({ ok: false, reply: "Our AI assistant is temporarily offline — please submit a ticket below and Mark will reply directly within one business day." })
  }
  try {
    ...
    const historyText = messages.slice(-6).map((m) => `${m.role === 'user' ? 'Client' : 'Assistant'}: ${m.content}`).join('\n')
...
Reply to the most recent client message. Use the KB above. If the client wants step-by-step setup, give numbered steps from the matching article. If the question isn't covered, say "I'll loop in Mark — please submit a ticket below."`
    const result = await callGemini({ model: GEMINI_FLASH, systemInstruction: SYSTEM_INSTRUCTION, parts: [{ text: prompt }], maxOutputTokens: 1024, temperature: 0.3 })
    if (!result.ok) {
      console.error('Ticketing chat error:', result.error)
      return NextResponse.json({ ok: false, reply: "I'm having trouble right now — please submit a ticket below and Mark will reply directly within one business day." })
    }
    return NextResponse.json({ ok: true, reply: result.text.trim() })
  } catch (err) {
    console.error('Ticketing chat exception:', err)
    return NextResponse.json({ ok: false, reply: "Something went wrong — please submit a ticket below and we'll reply directly." })
  }
}
